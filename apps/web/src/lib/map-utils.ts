...
export function calculateMapBounds(
  ...
  const intervalStart =
    largestGapIndex === sortedLongitudes.length - 1
      ? sortedLongitudes[0]
      : sortedLongitudes[largestGapIndex + 1];

  const adjustedLongitudes = longitudes.map((longitude) =>
    longitude < intervalStart ? longitude + 360 : longitude,
  );
...
export function calculateApproximateCoverageAreaKm2(
  ...
  const widthKm =
    Math.abs(bounds.longitudeSpan) *
    KM_PER_LATITUDE_DEGREE *
    Math.cos(midLatitude);
